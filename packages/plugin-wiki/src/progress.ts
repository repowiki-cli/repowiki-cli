...
export function createProgressReporter(opts: { quiet: boolean }): ProgressReporter {
  ...
  return process.stdout.isTTY ? createTtyReporter() : createCiReporter();
}
...
function createCiReporter(): ProgressReporter {
  return (event) => {
    switch (event.type) {
      case 'analyze:start':
        safeWrite('Analyzing repository...\n');
        break;
      case 'summarize-modules:start':
        safeWrite(`Summarizing ${event.total} modules...\n`);
        break;
      case 'summarize-parents:start':
        safeWrite(`Summarizing ${event.total} packages/directories...\n`);
        break;
      case 'write:done':
        safeWrite(`Written ${event.fileCount} wiki files\n`);
        break;
      case 'finished':
        safeWrite(
...
function createTtyReporter(): ProgressReporter {
  ...
  function writeProgress(text: string): void {
    const maxLen = Math.max(1, process.stdout.columns ?? 80) - 1;
  ...
  return (event) => {
    switch (event.type) {
      ...
      case 'write:done':
        writeLine(
          `✓ Written ${event.fileCount} wiki files (${(event.elapsed / 1000).toFixed(1)}s)`,
        );
        break;
      case 'finished':
        writeLine(
