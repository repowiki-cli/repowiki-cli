...
export async function writeHarnessBlock(filePath: string, innerContent: string): Promise<void> {
  ...
  if (startIdx !== -1 && endIdx === -1) {
    process.stderr.write(
      `[warn] Unclosed repowiki block found in \`${filePath}\`; removing orphaned start tag and appending new block\n`,
    );
